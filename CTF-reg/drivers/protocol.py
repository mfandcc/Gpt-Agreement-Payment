...
from mail.provider import MailProvider  # Wave H: mail_provider.py → mail/provider.py
...
def exchange_refresh_token_protocol(
    email: str,
    password: str,
    mail_cfg: dict,
    proxy_url: str = "",
    oauth_client_id: str = "",
    sms_bower_cfg: dict | None = None,
) -> str:
    """纯协议 RT 重登：与 card._exchange_refresh_token_with_session 函数签名对齐。

    `WEBUI_REG_MODE=protocol` 时由 pipeline 选用，避免在选了纯协议的会话里
    静默回落到 Camoufox。

    流程：构 Config + MailProvider → AuthFlow.run_protocol_login（sentinel +
    email OTP + Codex OAuth）→ 返回 result.refresh_token。失败 raise，让上层
    fail-fast 而不是吞掉退回浏览器。
    """
    from config import Config, MailConfig
    from mail.provider import MailProvider

    cfg = Config()
    cfg.mail = MailConfig(catch_all_domain=(mail_cfg or {}).get("catch_all_domain", "") or "")
    cfg.proxy = proxy_url or None
    cfg.sms_bower = sms_bower_cfg or {}

    if oauth_client_id and not (os.environ.get("OAUTH_CODEX_CLIENT_ID", "") or "").strip():
        os.environ["OAUTH_CODEX_CLIENT_ID"] = oauth_client_id
    os.environ.setdefault("OAUTH_REFRESH_ONLY", "1")

    mail = MailProvider(cfg.mail.catch_all_domain)

    # MailProvider.wait_for_otp 走 IMAP 的条件是 self._outlook_creds 有且 email 匹配。
    # 但事后补 RT 没调 create_mailbox（已有账号不创建邮箱），所以要根据目标邮箱手动从
    # outlook_accounts 表注入凭证；否则会 fallback 到 CF KV → outlook 邮箱永远收不到码。
    try:
        import sys as _sys
        from pathlib import Path as _Path
        _root = _Path(__file__).resolve().parents[2]
        ...
        if row and (row["refresh_token"] or "") and (row["client_id"] or ""):
            ...
            logger.info(f"[rt-protocol] 注入 outlook IMAP 凭证: {row['email']}")
    except Exception as e:
        logger.warning(f"[rt-protocol] 查询 outlook 凭证失败（将走 CF KV 兜底）: {e}")

    flow = AuthFlow(cfg)
    result = flow.run_protocol_login(mail, email, password)

    rt = (result.refresh_token or "").strip()
    if not rt:
        raise RuntimeError("protocol RT exchange 完成但 refresh_token 为空")
    return rt
